  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-white">{t('approvals.page.title')}</h1>
          <p className="text-sm text-[var(--github-text-secondary)] mt-1">{t('approvals.page.subtitle')}</p>
        </div>
      </div>

      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <TabsList className="bg-[var(--github-surface)] border border-[var(--github-border)]">
          <TabsTrigger value="pending"><Clock className="w-4 h-4 mr-2" />{t('approvals.tab.pending')} {pendingCount > 0 && (<Badge className="ml-1 bg-[var(--github-accent)] text-white text-xs">{pendingCount}</Badge>)}</TabsTrigger>
          <TabsTrigger value="approved"><CheckCircle className="w-4 h-4 mr-2" />{t('approvals.tab.approved')}</TabsTrigger>
          <TabsTrigger value="rejected"><XCircle className="w-4 h-4 mr-2" />{t('approvals.tab.rejected')}</TabsTrigger>
        </TabsList>
...
function DetailPanel({ approval, actionLoading, comment, setComment, editedContent, setEditedContent, onApprove, onReject, onEditAndApprove, readonly }: {
  ...
  return (
    <Card className="card-github sticky top-4">
      <CardHeader>
        <CardTitle className="text-base font-semibold text-white flex items-center gap-2"><GitPullRequest className="w-5 h-5" />{t('approvals.detail.title')}</CardTitle>
      </CardHeader>
      <CardContent className="space-y-4">
        <div>
          <h4 className="text-sm font-medium text-white mb-1">{t('approvals.detail.event')}</h4>
          <p className="text-sm text-[var(--github-text-secondary)]">{approval.event?.title || t('approvals.detail.unknown')}</p>
        ...
        <div>
          <h4 className="text-sm font-medium text-white mb-1">{t('approvals.detail.aiSummary')}</h4>
          <div className="p-3 rounded-lg bg-white/5 text-sm text-[var(--github-text)] max-h-40 overflow-y-auto">
            {approval.editedContent || approval.originalContent || t('approvals.detail.noAnalysis')}
          </div>
        </div>
        ...
        {!readonly && (
          <>
            <div>
              <h4 className="text-sm font-medium text-white mb-1">{t('approvals.detail.editContent')}</h4>
              <Textarea value={editedContent || ''} onChange={function (e) { if (setEditedContent) setEditedContent(e.target.value); }} placeholder={t('approvals.detail.editPlaceholder')} className="bg-[var(--github-surface)] border-[var(--github-border)] min-h-[100px]" />
            </div>
            <div>
              <h4 className="text-sm font-medium text-white mb-1">{t('approvals.detail.comment')}</h4>
              <Textarea value={comment || ''} onChange={function (e) { if (setComment) setComment(e.target.value); }} placeholder={t('approvals.detail.commentPlaceholder')} className="bg-[var(--github-surface)] border-[var(--github-border)]" />
            </div>
            <div className="flex gap-3">
              <Button onClick={onApprove} disabled={actionLoading} className="flex-1 gap-2 bg-green-600 hover:bg-green-700">{actionLoading ? <Spinner className="h-4 w-4" /> : <CheckCircle className="w-4 h-4" />}{t('approvals.detail.approve')}</Button>
              <Button onClick={onReject} disabled={actionLoading} variant="outline" className="flex-1 gap-2 border-red-400 text-red-400 hover:bg-red-400/10">{actionLoading ? <Spinner className="h-4 w-4" /> : <XCircle className="w-4 h-4" />}{t('approvals.detail.reject')}</Button>
            </div>
            {editedContent && editedContent !== approval.originalContent && (
              <Button onClick={onEditAndApprove} disabled={actionLoading || !editedContent} variant="outline" className="w-full gap-2">{actionLoading ? <Spinner className="h-4 w-4" /> : <MessageSquare className="w-4 h-4" />}{t('approvals.detail.editAndApprove')}</Button>
            )}
          </>
        )}
        ...
        {readonly && approval.comment && (
          <div>
            <h4 className="text-sm font-medium text-white mb-1">{t('approvals.detail.reviewComment')}</h4>
