...
async function main() {
  ...
  const users = await prisma.user.findMany({
    select: { id: true, email: true, name: true, githubLogin: true, githubId: true }
  });
  console.log(users);

  console.log('--- REPOSITORIES ---');
  const repos = await prisma.repository.findMany();
  console.log(repos.map(r => ({ id: r.id, fullName: r.fullName, isActive: r.isActive, canOperate: r.canOperate })));
